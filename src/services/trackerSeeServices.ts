import { ActionRowBuilder, AttachmentBuilder, ButtonBuilder, ButtonInteraction, ButtonStyle } from "discord.js";
...
export const see_happiness_message = async (int: ButtonInteraction) => {
    try{
        ...
        const data = await get_entries({userId: int.user.id}, 30, 4)
...
export const d7 = async (int: ButtonInteraction) => {
    try {
      ...
      const data = await get_entries({ userId: int.user.id }, 7, 7);
...
export const m3 = async (int: ButtonInteraction) => {
    try {
      ...
      const data = await get_entries({ userId: int.user.id }, 90, 3);
...
export const m6 = async (int: ButtonInteraction) => {
    try {
      ...
      const data = await get_entries({ userId: int.user.id }, 180, 6);
      const graph = await create_entry_graph(data, months);
      const attachment = new AttachmentBuilder(graph);
